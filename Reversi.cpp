...
int main() {
    ...
    while (compHasMove == true || playerHasMove == true) {
        if (currentMove == sideComputer) currentMove = computerMove(board, n, currentMove, &compHasMove);
        else {
            currentMove = playerMove(board, n, currentMove, &playerHasMove, &playerMistake);
            if (playerMistake) break;
        }
    }
    if (playerMistake) printf("%c player wins.\n", sideComputer);
    else getWinner(board, n);
    return 0;
}
...
void initialBoard(char board[][26], int n) {
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < n; j++) {
            if (i == n / 2 - 1 && j == n / 2 - 1 || i == n / 2 && j == n / 2) board[i][j] = 'W';
            else if (i == n / 2 && j == n / 2 - 1 || i == n / 2 - 1 && j == n / 2) board[i][j] = 'B';
            else board[i][j] = 'U';
        }
    }
    printBoard(board, n);
        ...
        for (char col = 'a'; col < 'a' + n; col++) {
            if (board[(int)(row - 'a')][(int)(col - 'a')] == 'U') {
                ...
                if (positionLegal) {
                    ...
                    if (n >= 6) {
                        //if (row == 'a' + 1 || row == 'a' + n - 2 || col == 'a' + 1 || col == 'a' + n - 2) score = score - 1;
...
bool checkLegalInDirection(char board[][26], int n, char row, char col,
    ...
    while (!legal) {
        ...
        if (board[(int)(row - 'a')][(int)(col - 'a')] == colour && board[(int)(row - 'a') - deltaRow][(int)(col - 'a') - deltaCol] == opcolour) legal = true;
        else if (board[(int)(row - 'a')][(int)(col - 'a')] == colour && board[(int)(row - 'a') - deltaRow][(int)(col - 'a') - deltaCol] != opcolour
            || board[(int)(row - 'a')][(int)(col - 'a')] == 'U' || !inBounds) break;
    }
    return legal;
}
...
bool positionInBounds(char board[][26], int n, char row, char col) {
    bool inBounds;
    if (row >= 'a' && row <= 'a' + n - 1 && col >= 'a' && col <= 'a' + n - 1) inBounds = true;
    else inBounds = false;
    return inBounds;
}
